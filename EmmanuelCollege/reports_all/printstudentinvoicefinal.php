...
$sql = "SELECT reciept AS Reciept_No, method AS Method, refno AS Ref_No, tdate AS Tdate, amount AS Amount FROM fee  WHERE admission_number='$id' AND course_id='$form'";
$resultset = mysqli_query($db, $sql) or die("database error:". mysqli_error($db));
//get the students details
$sql2 = "SELECT * FROM studentstable  WHERE admission_number='$id'";
...
while($users3=mysqli_fetch_array($resultset2,MYSQLI_ASSOC))
        ...
        $admission_number=$users3['admission_number'];
...
$sql="SELECT SUM(amount) AS value_sum FROM fee WHERE  admission_number='$id' AND course_id='$form'";
$user_query=mysqli_query($db,$sql) or die("error getting data");
while($row = mysqli_fetch_array($user_query, MYSQLI_ASSOC)){
...
$pdf->SetFontSize(10);//set the font size of the last line
$pdf->SetFont('','u');
$pdf->Write(10,'Courses offered: Information Technology, Beauty & Hairdressing, Business Studies, Catering and Tourism');
$pdf->SetTextColor(0,0,0);
